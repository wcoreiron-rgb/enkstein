...
PROVIDER_MAP = [
    {"provider": "terraform_mcp",   "label": "Terraform MCP",    "connector_type": "terraform_mcp"},
...
''',
    "gcp_cloud_sql": '''# Secure Google Cloud SQL PostgreSQL Instance
# Generated by TerraClaw — security-first defaults applied
...
resource "google_sql_database_instance" "main" {
  ...
  settings {
    ...
    ip_configuration {
      # TC-NET-004: Disable public IPv4 and use private service access
      ipv4_enabled    = false
...
resource "google_sql_user" "app" {
  name     = var.app_user
  instance = google_sql_database_instance.main.name
  password = var.app_password # Use Secret Manager in production
}
''',
}
...
_TEMPLATE_METADATA = {
    "azure_sql": {
        "name": "Secure Azure SQL Server with Private Endpoint",
        "cloud": "azure",
        "resources": ["azurerm_mssql_server", "azurerm_mssql_database", "azurerm_private_endpoint"],
        "controls": ["TC-NET-004", "TC-DATA-003", "TC-MON-001"],
        "frameworks": ["CIS Azure 4.1", "NIST SC-7", "SOC2 CC6.6"],
    },
    "azure_storage": {
        "name": "Secure Azure Storage Account",
        "cloud": "azure",
        "resources": ["azurerm_storage_account", "azurerm_monitor_diagnostic_setting"],
        "controls": ["TC-DATA-001", "TC-DATA-002", "TC-DATA-004", "TC-MON-001"],
        "frameworks": ["CIS Azure 3.1", "CIS Azure 3.7", "NIST SC-8", "SOC2 CC6.7"],
    },
    "aws_rds": {
        "name": "Secure AWS RDS PostgreSQL",
        "cloud": "aws",
        "resources": ["aws_db_subnet_group", "aws_security_group", "aws_db_instance"],
        "controls": ["TC-NET-003", "TC-DATA-003", "TC-MON-002"],
        "frameworks": ["CIS AWS 2.3.1", "NIST SC-28", "SOC2 CC6.1", "PCI-DSS 3.4"],
    },
    "aws_ec2": {
        "name": "Secure AWS EC2 with SSM",
        "cloud": "aws",
        "resources": ["aws_security_group", "aws_iam_role", "aws_instance"],
        "controls": ["TC-NET-001", "TC-NET-002", "TC-DATA-003"],
        "frameworks": ["CIS AWS 4.1", "NIST AC-17", "SOC2 CC6.6"],
    },
    "aks": {
        "name": "Private AKS Cluster with Defender",
        "cloud": "azure",
        "resources": ["azurerm_kubernetes_cluster", "azurerm_user_assigned_identity"],
        "controls": ["TC-NET-005", "TC-MON-001", "TC-IAM-002"],
        "frameworks": ["CIS Azure 8.2", "NIST SC-7", "SOC2 CC6.6"],
    },
    "gcp_cloud_sql": {
        "name": "Secure Google Cloud SQL PostgreSQL",
        "cloud": "gcp",
        "resources": ["google_sql_database_instance", "google_sql_database", "google_sql_user"],
        "controls": ["TC-NET-004", "TC-DATA-003", "TC-MON-001"],
        "frameworks": ["CIS GCP 6.2", "NIST SC-7", "NIST SC-28", "SOC2 CC6.1"],
    },
}


def _generation_artifacts(template_key: str, terraform: str) -> list[dict]:
    variables = sorted(set(re.findall(r"\bvar\.([A-Za-z0-9_]+)", terraform)))
    return [
        {"path": "main.tf", "type": "terraform_hcl", "bytes": len(terraform.encode("utf-8"))},
        {"path": "variables.tf", "type": "terraform_variables", "variables": variables},
        {"path": "README.md", "type": "operator_notes", "template": template_key},
    ]
...
class GenerateRequest(BaseModel):
    ...
    workspace: str = Field(default="default", max_length=120)
    environment: str = Field(default="dev", max_length=80)
    output_mode: str = Field(default="module", pattern=r"^(module|single_file|plan_ready)$")
    classification: str = Field(default="internal", max_length=64)
...
async def get_findings(db: AsyncSession = Depends(get_db)):
    ...
    if not findings:
        ...
        if not any_configured or not has_live_adapter(PROVIDER_MAP):
            # No adapter here can return tenant data, so labelled demonstration
            # findings are more honest than an empty module.
            return [
                {
                    **f,
...
async def get_providers(db: AsyncSession = Depends(get_db)):
    ...
    for provider in providers:
        if provider["provider"] == "terraform_mcp":
            provider["capabilities"] = [
                "generate_secure_module",
                "review_hcl",
                "analyze_plan",
                "map_controls",
            ]
        ...
        elif provider["provider"] in {"tfsec", "checkov"}:
            provider["capabilities"] = ["static_iac_scan", "rule_evidence"]
            provider["mode"] = "connector" if provider["configured"] else "local_rule_fallback"
        elif provider["provider"] == "infracost":
            provider["capabilities"] = ["cost_delta", "budget_signal"]
            provider["mode"] = "connector" if provider["configured"] else "not_configured"
    return providers
...
async def generate_terraform(
    ...
    for finding in findings:
        for fw in finding.get("frameworks", []):
            framework_hits[fw] = framework_hits.get(fw, 0) + 1
    if not framework_hits:
        framework_hits = {fw: 0 for fw in template_meta.get("frameworks", [])}
    ...
    return {
        ...
        "agent_trace": [
            {
                ...
                "status": "completed",
                "detail": f"Selected {template_meta['name']} for {body.cloud.upper()} {body.environment}.",
            },
            {
                "step": "fabric_gate",
                "status": "completed",
                "detail": f"{policy_decision.outcome.value} by {policy_decision.policy_name}.",
            },
            {
                "step": "terraform_mcp",
                "status": "completed",
            ...
            },
            {
                ...
                "status": "completed",
                "detail": f"{len(findings)} findings, risk score {risk_score}/100.",
            },
        ],
        "artifacts": _generation_artifacts(template_key, terraform),
        "controls_applied": template_meta.get("controls", []),
        "frameworks_impacted": framework_hits,
        "security_review": {
            "finding_count": len(findings),
            "findings": findings,
        ...
        "notes": [
            ...
            "For CI/CD, attach this generate_id and the terraform plan output to ReleaseClaw before apply.",
        ],
        "next_actions": [
            {"label": "Copy Terraform", "target": "clipboard", "type": "operator"},
            {"label": "Run Review", "target": "/api/v1/terraclaw/review", "type": "api"},
            {"label": "Analyze Plan", "target": "/api/v1/terraclaw/plan", "type": "api"},
            {"label": "Gate Release", "target": "/api/v1/releaseclaw/gate", "type": "api"},
        ],
        "execution_time_ms": elapsed_ms,
    }
