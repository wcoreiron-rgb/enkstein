#!/usr/bin/env bash
# Re-mirror the standalone governance modules from the backend into enkstein-core.
# Run from repo root after changing any of the source modules.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
DST="$ROOT/enkstein-core/enkstein_core"
declare -A MAP=(
...
for src in "${!MAP[@]}"; do
  ...
  { echo "# NOTE: mirrored from ${src#$ROOT/} — keep in sync via scripts/sync_core.sh."
    ...
    echo "# can run embedded with no Enkstein server."
    cat "$src"; } > "$dst"
  echo "synced ${MAP[$src]}"
done
