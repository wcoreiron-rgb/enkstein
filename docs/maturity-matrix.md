...
| Command/channel control plane convergence | In Progress | CommandClaw remote-agent routes + channel ingress normalized to unified command contract (`POST /api/v1/commands`) with policy outcome propagation, simulate-path parity, remote dispatch guardrails (tenant/kill-switch/intent allowlist), webhook/email/CLI ingress adapters, command pending/approve endpoints, and frontend pending-command approval UX in Channel Gateway | Strengthen policy-state persistence semantics and expand multi-operator approval controls |
