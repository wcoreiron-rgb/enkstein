...
Version 0.5.7 verifies Codex with `codex login status` and Claude Code with
...
```bash
./scripts/build_macos_pkg.sh 0.5.7
open dist/Enkstein-0.5.7-macos.pkg
```

The output is `dist/Enkstein-0.5.7-macos.pkg`. Local builds use ad-hoc app
