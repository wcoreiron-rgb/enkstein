...
Version 0.6.4 verifies Codex with `codex login status` and Claude Code with
...
```bash
./scripts/build_macos_pkg.sh 0.6.4
open dist/Enkstein-0.6.4-macos.pkg
```

The output is `dist/Enkstein-0.6.4-macos.pkg`. Local builds use ad-hoc app
