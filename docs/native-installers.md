...
Version 0.6.3 verifies Codex with `codex login status` and Claude Code with
...
```bash
./scripts/build_macos_pkg.sh 0.6.3
open dist/Enkstein-0.6.3-macos.pkg
```

The output is `dist/Enkstein-0.6.3-macos.pkg`. Local builds use ad-hoc app
