...
Version 0.3.9 verifies Codex with `codex login status` and Claude Code with
...
```bash
./scripts/build_macos_pkg.sh 0.3.9
open dist/Enkstein-0.3.9-macos.pkg
```

The output is `dist/Enkstein-0.3.9-macos.pkg`. Local builds use ad-hoc app
