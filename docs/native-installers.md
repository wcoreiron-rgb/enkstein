...
Version 0.3.12 verifies Codex with `codex login status` and Claude Code with
...
```bash
./scripts/build_macos_pkg.sh 0.3.12
open dist/Enkstein-0.3.12-macos.pkg
```

The output is `dist/Enkstein-0.3.12-macos.pkg`. Local builds use ad-hoc app
